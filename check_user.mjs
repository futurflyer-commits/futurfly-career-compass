...
async function check() {
  ...
  if (err1) console.error('Auth check error:', err1);
  ...
  // Try to query likely skill tables
  ...
  for (const table of tables) {
    ...
    if (!error && data && data.length > 0) {
      ...
      // PGRST204 is usually related to relation not found but could be 42P01 table not found
      // Only print if it's not "table does not exist"
